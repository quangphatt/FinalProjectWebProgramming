...
<body class="body">
    ...
    <main class="container">
	    ...
	    <br />
	    	<ul class="menu-strip">
	    		<li><button id="button1">Booking Accepted</button></li>
	    		<li><button id="button2">Booking Waiting</button></li>
	    		<li><button id="button3">History</button></li>
	    	</ul>
	    </section>
	    <section class="menu-right">
	    <div class="now">
	    	<h1>Booking Have Accepted</h1>
	    	<table class="tables table table-stripe">
	    		...
	    		<c:forEach var="item" items="${book_now}">
					<tr>
						<td>${item.id}</td>
						<td>${item.email}</td>
						...
						<td>
							<a href="<c:url value='/Manage/ManageBookingView?action=view&ID=${item.id}'></c:url>">View</a>
							<a href="<c:url value='/Manage/ManageBooking?action=finished&ID=${item.id}'></c:url>">Finished</a>						
						</td>
					</tr>
				</c:forEach>
	    	</table>
	    ...
	    <div class="wait">
	    	...
	    	<table class="tables table table-stripe" >
	    		...
	    		<c:forEach var="item" items="${book_wait}">
					<tr>
						<td>${item.id}</td>
						<td>${item.email}</td>
						<td>${item.date}&nbsp;${item.time}</td>
						...
						<td>
							<a href="<c:url value='/Manage/ManageBookingView?action=view&ID=${item.id}'></c:url>">View</a>
							<a href="<c:url value='/Manage/ManageBooking?action=accept&ID=${item.id}'></c:url>">Accept</a>	
							<a href="<c:url value='/Manage/ManageBooking?action=deny&ID=${item.id}'></c:url>">Deny</a>						
	    ...
	    <div class="history">
	    	...
	    	<table class="tables table table-stripe">
	    		...
	    		<c:forEach var="item" items="${book_history}">
					<tr>
						<td>${item.id}</td>
						<td>${item.email}</td>
						<td>${item.date}&nbsp;${item.time}</td>
